...
const AppointmentForm = ({ selectedDate, onClose, onSuccess, appointmentToEdit }: AppointmentFormProps) => {
  ...
  const {
    ...
    onClose,
    onSuccess,
    // Pass services from RTK Query instead of fetching in the hook
  ...
  const {
    customers,
    // services, // Now comes from RTK Query
    availableEmployees,
  ...
  const { availableTimeSlots, isLoadingAppointments } = useAvailableTimeSlots({
    ...
    appointmentDate,
    selectedServiceDuration,
    services, // Pass RTK Query services here
    selectedEmployeeWorkingHours,
    isEditMode,
  ...
  const error = formError || (servicesError ? 'Hizmetler yüklenirken bir hata oluştu' : null);
  const isLoading = formLoading || servicesLoading;
  ...
  // Handle services error with retry option
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="col-span-1">
            ...
            {/* Service-specific error/status messages */}
            {servicesError && (
          ...
          <div className="col-span-1">
            ...
            <div className="relative">
              <select
                ...
                required
                disabled={isLoading || !serviceId || availableEmployees.length === 0}
              >
                <option value="">
            ...
            {serviceId && availableEmployees.length === 0 && !isLoading && (
